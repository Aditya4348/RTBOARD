...
const DataWargaPage: React.FC = () => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    setWarga((prevWarga) => {
      ...
      return newWarga.sort((a, b) =>
        a.kepalaKeluarga.localeCompare(b.kepalaKeluarga)
      );
    });
  ...
  const handleAnggotaChange = (
    ...
    if (
      field === "nama" &&
      newAnggota[index].statusHubungan === "Kepala Keluarga"
    ) {
  ...
  // Mengambil semua anggota dari semua KK
  const semuaAnggota = warga.flatMap((w) => w.anggota);

  // 1. Jumlah total jiwa / penduduk
  const totalJiwa = semuaAnggota.length;

  // 2. Distribusi usia (asumsikan tanggalLahir format YYYY-MM-DD)
  ...
  const usiaArray = semuaAnggota
    .map((a) => getAge(a.tanggalLahir))
    .filter((a) => a !== null);

  // Misal ingin distribusi usia per kelompok
  ...
  semuaAnggota.forEach((a) => {
    if (a.jenisKelamin)
      jenisKelamin[a.jenisKelamin] = (jenisKelamin[a.jenisKelamin] || 0) + 1;
  });
  ...
  semuaAnggota.forEach((a) => {
    if (a.pendidikan)
      pendidikan[a.pendidikan] = (pendidikan[a.pendidikan] || 0) + 1;
  });

  console.log("Total jiwa:", totalJiwa);
  console.log("Distribusi usia:", kelompokUsia);
  ...
  const handleFileChange = async (
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        const textPart = {
          text: "Ekstrak semua informasi dari gambar Kartu Keluarga ini. Pastikan NIK dan No. KK 16 digit. Format tanggal lahir YYYY-MM-DD.",
        };
        ...
        if (parsedData.anggota) {
          parsedData.anggota = parsedData.anggota.map((a) => ({
            ...a,
        ...
        alert(
          "Gagal memproses gambar KK. Pastikan gambar jelas dan coba lagi."
        );
      } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal
        ...
        title={isEditing ? "Edit Data Warga" : "Tambah Data Warga"}
        size="xl"
      >
        {currentWarga && (
          <form onSubmit={handleSave} className="space-y-6">
            <fieldset className="border border-slate-300 dark:border-slate-600 rounded-lg p-4">
              <legend className="px-2 font-semibold text-slate-700 dark:text-slate-300">
                Data Kartu Keluarga
              </legend>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={currentWarga.id}
                    onChange={(e) =>
                      setCurrentWarga({ ...currentWarga, id: e.target.value })
                    }
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={currentWarga.kepalaKeluarga}
                    onChange={(e) =>
                      setCurrentWarga({
                        ...currentWarga,
                        kepalaKeluarga: e.target.value,
                      })
                    }
                    className="mt-1 block w-full input-style"
                    required
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:col-span-2">
                  <div>
                    <label className="label-style">RT</label>
                    <input
                      type="text"
                      value={currentWarga.rt}
                      onChange={(e) =>
                        setCurrentWarga({ ...currentWarga, rt: e.target.value })
                      }
                      className="input-style mt-1"
                    />
                  </div>
                  <div>
                    <label className="label-style">RW</label>
                    <input
                      type="text"
                      value={currentWarga.rw}
                      onChange={(e) =>
                        setCurrentWarga({ ...currentWarga, rw: e.target.value })
                      }
                      className="input-style mt-1"
                    />
                  </div>
                  <div>
                    <label className="label-style">Kode Pos</label>
                    <input
                      type="text"
                      value={currentWarga.kodePos}
                      onChange={(e) =>
                        setCurrentWarga({
                          ...currentWarga,
                ...
                <div className="md:col-span-2">
                  ...
                  <input
                    type="text"
                    value={currentWarga.kelurahanDesa}
                    onChange={(e) =>
                      setCurrentWarga({
                        ...currentWarga,
                        kelurahanDesa: e.target.value,
                      })
                    }
                    className="input-style mt-1"
                  />
                ...
                <div>
                  <label className="label-style">Kabupaten/Kota</label>
                  <input
                    type="text"
                    value={currentWarga.kotaKabupaten}
                    onChange={(e) =>
                      setCurrentWarga({
                        ...currentWarga,
                ...
                <div>
                  <label className="label-style">Provinsi</label>
                  <input
                    type="text"
                    value={currentWarga.provinsi}
                    onChange={(e) =>
                      setCurrentWarga({
                        ...currentWarga,
            ...
            <fieldset className="border border-slate-300 dark:border-slate-600 rounded-lg p-4">
              ...
              <div className="space-y-4">
                {currentWarga.anggota.map((anggota, index) => (
                  <div
                    key={anggota.id}
                    className="p-4 border rounded-md space-y-3 dark:border-slate-600 bg-slate-50 dark:bg-slate-800/50"
                  >
                    <div className="flex justify-between items-center">
                      <span className="font-medium text-slate-800 dark:text-slate-200">
                        Anggota {index + 1}
                      </span>
                      {currentWarga.anggota.length > 1 && (
                        <button
                          type="button"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={anggota.nik}
                          onChange={(e) =>
                            handleAnggotaChange(index, "nik", e.target.value)
                          }
                          className="input-style mt-1"
                          required
                        />
                      ...
                      <div>
                        <label className="label-style">Agama</label>
                        <input
                          type="text"
                          value={anggota.agama}
                          onChange={(e) =>
                            handleAnggotaChange(index, "agama", e.target.value)
                          }
                          className="input-style mt-1"
                        />
                      </div>
                      ...
                      <div>
                        <label className="label-style">Jenis Pekerjaan</label>
                        <input
                          type="text"
                          value={anggota.jenisPekerjaan}
                          onChange={(e) =>
                            handleAnggotaChange(
                              index,
                              "jenisPekerjaan",
                      ...
                      <div className="md:col-span-2">
                        <label className="label-style">Status Hubungan</label>
                        <input
                          type="text"
      ...
      >
        <div className="text-center">
          ...
          <h3 className="text-lg leading-6 font-medium text-slate-900 dark:text-white mt-4">
            Unggah Foto KK
          </h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
            Pilih foto Kartu Keluarga yang jelas dan tidak buram untuk hasil
            terbaik.
          </p>
          <div className="mt-6">
            <input
            ...
            <label
              htmlFor="kk-upload"
              className="cursor-pointer inline-flex justify-center w-full rounded-lg border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:text-sm"
            >
              Pilih File Gambar
            </label>
          </div>
